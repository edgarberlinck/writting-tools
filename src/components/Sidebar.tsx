  ...
  return (
    <div className="w-64 bg-gray-800 flex flex-col h-full border-r border-gray-700 flex-shrink-0">
          ...
          <SortableContext items={chapters.map((c) => c._id)} strategy={verticalListSortingStrategy}>
            {chapters.map((chapter) => (
              <ChapterItem
                ...
                selectedSectionId={selectedChapterId === chapter._id ? selectedSectionId : null}
                onSelect={() => props.onSelectChapter(chapter._id)}
                onSelectSection={(sid) => props.onSelectSection(chapter._id, sid)}
                ...
                onReorderSections={(ids) => props.onReorderSections(chapter, ids)}
              />
            ))}
          </SortableContext>
        </DndContext>
      ...
      <div className="p-3 border-t border-gray-700 relative">
        ...
        {showTypeMenu && (
          <div className="absolute bottom-full left-3 right-3 mb-1 bg-gray-700 rounded-lg shadow-lg overflow-hidden z-10">
            {CHAPTER_TYPES.map((t) => (
              <button
                key={t.value}
                onClick={() => { props.onAddChapter(t.value); setShowTypeMenu(false); }}
